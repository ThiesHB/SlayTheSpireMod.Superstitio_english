package SuperstitioMod.powers.patch;

import SuperstitioMod.powers.AbstractLupaPower;
            ...
            return new ExprEditor() {
                public void edit(MethodCall m) throws CannotCompileException {
                    if (m.getClassName().equals(ReflectionHacks.class.getName()) && m.getMethodName().equals("getPrivateStatic")) {
                        m.replace("if ( ! " + SuperstitioMod.powers.AbstractLupaPower.class.getName() + ".class.isAssignableFrom ( $1 ) )" +
                                " {  $_ = $proceed($$); } ");
