...
class SexualHeat protected constructor(owner: AbstractCreature, private var heatAmount: Int) : AbstractSuperstitioPower(
    ...
    InvisiblePower_InvisibleRemovePowerEffect, OnOrgasm_onSuccessfullyPreventOrgasm, OnOrgasm_onOrgasm,
    ...
    override fun makeMessage(): BarRenderUpdateMessage {
        return super.makeMessage().setDetail { bar: RenderOnThing? ->
            if (bar is BarRenderOnThing) bar.barShadowColor =
                if (this.isInOrgasm) this.barOrgasmShadowColor else this.barShadowColorOrigin
        }
    }
...
//            this.addToBot(new ReducePowerAction(this.owner, this.owner, POWER_ID, this.amount));
    ...
    override fun onEndOrgasm(SexualHeatPower: SexualHeat) {
        ...
        val power = this
        AutoDoneInstantAction.addToBotAbstract { AllCardCostModifier.RemoveAllByHolder(power) }
    }
    ...
    companion object {
        ...
        fun getActiveSexualHeat(creature: AbstractCreature): SexualHeat? {
            return creature.powers.stream()
                .filter { power: AbstractPower? -> power is SexualHeat }.findAny().orElse(null) as SexualHeat?
        }
