...
//        }
    ...
    protected val totalSemenValue: Int
        get() {
            val insideSemenValue: Int = AbstractDungeon.player.powers.stream()
                .filter { power: AbstractPower? -> power is InsideSemen }
                .map(AbstractPower::amount)
                .findAny().orElse(0) * FuckJob_Card.InsideSemenRate
            val outsideSemenValue: Int = AbstractDungeon.player.powers.stream()
                .filter { power: AbstractPower? -> power is OutsideSemen }
                .map(AbstractPower::amount)
                .findAny().orElse(0) * FuckJob_Card.OutsideSemenRate
            val floorSemenValue: Int = AbstractDungeon.player.powers.stream()
                .filter { power: AbstractPower? -> power is FloorSemen }
                .map(AbstractPower::amount)
                .findAny().orElse(0) * FuckJob_Card.FloorSemenRate
