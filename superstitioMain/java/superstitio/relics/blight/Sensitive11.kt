...
@Seen
class Sensitive : SuperstitioRelic(ID, RELIC_TIER, LANDING_SOUND), BecomeInfoBlight, OnOrgasm_onOrgasmFirst {
    private val hpLose = 0

    override fun onOrgasmFirst(SexualHeatPower: SexualHeat) {
        val decreaseCost = min(
            SexualHeat.Companion.getOrgasmTimesInTurn().toDouble(),
        ...
        AutoDoneInstantAction.addToBotAbstract {
            try {
                AllCardCostModifier.addTo_Bot_EditAmount_Top_FirstByHolder(
                    SexualHeatPower, decreaseCost, { power: AllCardCostModifier? ->
                        if (power != null) return@addTo_Bot_EditAmount_Top_FirstByHolder 1
                        else return@addTo_Bot_EditAmount_Top_FirstByHolder 1
