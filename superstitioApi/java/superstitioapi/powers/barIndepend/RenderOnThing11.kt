...
abstract class RenderOnThing(protected val hitboxBondTo: Supplier<Hitbox>, power: HasBarRenderOnCreature) {
    ...
    protected val maxBarAmount: Int
        get() = amountChunkWithUuid.values.stream().mapToInt(AmountChunk::maxAmount).max().orElse(10)
    ...
    protected val nextOrder: Int
        get() = amountChunkWithUuid.values.stream().mapToInt(AmountChunk::order).max().orElse(-1) + 1
