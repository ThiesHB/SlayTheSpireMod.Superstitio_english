...
abstract class AllCardCostModifier(
    ...
    override fun InitializePostApplyThisPower(addedPower: AllCardCostModifier) {
        this.order = all.mapNotNull { it.order }.minOrNull() ?: 0
        addToBot_TryActivateLowestOrder()
    }
    ...
    companion object {
        ...
        fun <T : AllCardCostModifier?> CombineAllByHolder(aimHolder: HasAllCardCostModifyEffect, tClass: Class<T>) {
            ...
            val minOrderByHolder = cardCostModifier.stream().map { power: AllCardCostModifier? -> power!!.order }
                .min { obj: Int, anotherInteger: Int -> obj.compareTo(anotherInteger) }.orElse(0)
            cardCostModifier.firstOrNull { power: AllCardCostModifier? -> power!!.order == minOrderByHolder }
                .let { power: AllCardCostModifier? ->
                    val decreasedCost =
                        cardCostModifier.stream().mapToInt { power2: AllCardCostModifier? -> power2!!.decreasedCost }
                            .max().orElse(0)
                    val totalEnergy =
                        cardCostModifier.stream().mapToInt { power2: AllCardCostModifier? -> power2!!.amount }.sum()
                    power!!.decreasedCost = decreasedCost
                    power.amount = totalEnergy
                    cardCostModifier.stream().filter { power2: AllCardCostModifier? -> power2!!.order != power.order }
                        .forEach { obj: AllCardCostModifier? -> obj!!.removeSelf() }
                }
        }
    }
}
